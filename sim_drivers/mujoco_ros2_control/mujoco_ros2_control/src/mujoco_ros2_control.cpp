...
void MujocoRos2Control::init()
{
  ...
  // SSRVodka's NOTE:
  // Only after mujoco_system->init_sim can we record the initiali states.
  // Because mujoco_system will call set_initial_pose to set pose in mjData then.
  RCLCPP_INFO(logger_, "Copying initial states...");
  initial_qpos_.resize(mj_model_->nq);
  initial_qvel_.resize(mj_model_->nv);
  initial_act_.resize(mj_model_->na);
  initial_mocap_pose_.resize(3 * mj_model_->nmocap);
  initial_mocap_quat_.resize(4 * mj_model_->nmocap);
  initial_userdata_.resize(mj_model_->nuserdata);
  initial_qacc_warmstart_.resize(mj_model_->nv);

  // copy simulation state (state vector)
  mju_copy(initial_qpos_.data(), mj_data_->qpos, mj_model_->nq);
  mju_copy(initial_qvel_.data(), mj_data_->qvel, mj_model_->nv);
  mju_copy(initial_act_.data(), mj_data_->act, mj_model_->na);
  // copy mocap body pose and userdata
  mju_copy(initial_mocap_pose_.data(), mj_data_->mocap_pos, 3*mj_model_->nmocap);
  mju_copy(initial_mocap_quat_.data(), mj_data_->mocap_quat, 4*mj_model_->nmocap);
  mju_copy(initial_userdata_.data(), mj_data_->userdata, mj_model_->nuserdata);
  // copy warm-start acceleration
  mju_copy(initial_qacc_warmstart_.data(), mj_data_->qacc_warmstart, mj_model_->nv);

  mj_data_bak_ = mj_copyData(NULL, mj_model_, mj_data_);
...
void MujocoRos2Control::reset()
{
  // Reset MuJoCo simulation to initial state
  mju_copy(mj_data_->qpos, initial_qpos_.data(), mj_model_->nq);
  mju_copy(mj_data_->qvel, initial_qvel_.data(), mj_model_->nv);
  mju_copy(mj_data_->act, initial_act_.data(), mj_model_->na);
  mju_copy(mj_data_->mocap_pos, initial_mocap_pose_.data(), 3*mj_model_->nmocap);
  mju_copy(mj_data_->mocap_quat, initial_mocap_quat_.data(), 4*mj_model_->nmocap);
  mju_copy(mj_data_->userdata, initial_userdata_.data(), mj_model_->nuserdata);
  mju_copy(mj_data_->qacc_warmstart, initial_qacc_warmstart_.data(), mj_model_->nv);

  // clear controls and applied forces
  mju_zero(mj_data_->ctrl, mj_model_->nu);
  ...
  // Forward the simulation to update dependent variables
  mj_forward(mj_model_, mj_data_);
  
  RCLCPP_INFO(logger_, "MuJoCo environment reset to initial state");
}
