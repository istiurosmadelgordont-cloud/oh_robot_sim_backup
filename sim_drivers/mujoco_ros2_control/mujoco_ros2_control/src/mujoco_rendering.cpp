...
MujocoRendering::~MujocoRendering()
{
  for (const auto &_cam: off_cams) {
    delete _cam.second;
  }
}

int MujocoRendering::register_offscreen_camera(
  const std::string cam_name, int width, int height, int maxgeom)
{  
  int cam_id_ = mj_name2id(mj_model_, mjOBJ_CAMERA, cam_name.c_str());
  // cannot find camera with name ${cam_name} in mujoco model
  if (cam_id_ < 0) return cam_id_;
  // repeated registration
  if (off_cams.find(cam_id_) != off_cams.end()) return -2;

  OffscreenCamera_t *off_cam = new OffscreenCamera_t;
  // initialize off-screen renderer
  mjv_defaultScene(&off_cam->scn_);
  mjv_defaultOption(&off_cam->opt_);
  mjv_defaultCamera(&off_cam->cam_);
  mjr_defaultContext(&off_cam->con_);
  off_cam->cam_.type = mjCAMERA_FIXED;
  off_cam->cam_.fixedcamid = cam_id_; // valid when cam_type == mjCAMERA_FIXED
  off_cam->viewport_ = {0 /* left */, 0 /* bottom */, width, height};
  mjv_makeScene(mj_model_, &off_cam->scn_, maxgeom);
  // single OpenGL context (no window)
  mjr_makeContext(mj_model_, &off_cam->con_, mjFONTSCALE_100);

  off_cams.emplace(cam_id_, off_cam);
  return cam_id_;
}

void MujocoRendering::render_offscreen_camera(int cam_id, unsigned char *buf, float *depth)
{
  auto iter = off_cams.find(cam_id);
  // camera not registered
  if (iter == off_cams.end()) return;

  OffscreenCamera_t *off_cam = iter->second;
  // build scene for this camera
  mjv_updateScene(mj_model_, mj_data_,
                  &off_cam->opt_, nullptr,
                  &off_cam->cam_, mjCAT_ALL,
                  &off_cam->scn_);
  // render into the off-screen buffer
  mjr_render(off_cam->viewport_, &off_cam->scn_, &off_cam->con_);
  // read back the pixels
  mjr_readPixels(buf, depth, off_cam->viewport_, &off_cam->con_);
}
